...
namespace  eyetracking
{
    public partial class MainForm : Form
    {
        ...
        private int _calib_flag = 0;
        private int _picture_flag = 0;
        ...
        private void UpdateUIElements()
        {
            ...
            if(_connectedTracker !=  null)
            {
                ...
                _GazeButton.Enabled = true;
                _calib5.Enabled = true;
                _calib9.Enabled = true;
                _selectpicture.Enabled = true;
            }
            else
            {
                ...
                _GazeButton.Enabled = false;
                _calib5.Enabled = false;
                _calib9.Enabled = false;
 //               _selectpicture.Enabled = false;
            }
        ...
        private void _calibrateButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var result = runner.RunCalibration(_connectedTracker, _calib_flag);
        ...
        private void _GazeButton_Click(object sender, EventArgs e)          //Gaze
        {
            if (_isTracking)
            {
                _connectedTracker.StopTracking();
            ...
            var gp = new GazePoints(_picture_flag);
            try
            {
                gp.run(_connectedTracker);
            }
            catch (EyeTrackerException ee)
            {
                MessageBox.Show("Failed to calibrate. Got exception " + ee,
        ...
        private void _dammyButton_Click(object sender, EventArgs e)
        {
            var dp = new DammyPoints(_picture_flag);
            dp.run();
        }

        private void _confirmButton_Click(object sender, EventArgs e)           //confirm
        {
            if (_isTracking)
            {
                _connectedTracker.StopTracking();
            ...
            //キャリブレーション確認のため，４点を画面上に表示　＆　ログとり
            var conf = new CalibConfirm();
            try
            {
                //確認
                conf.run(_connectedTracker);
            }
            catch (EyeTrackerException ee)
            {
                MessageBox.Show("Failed to calibrate. Got exception " + ee,
        ...
        private void openFileToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                if (_openpictureFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    */
                    ...
                    pb.Image = new Bitmap(_openpictureFileDialog.FileName);
                    Console.WriteLine(pb.Image);
                }
            }
            catch (EyeTrackerException ee)
            {
                MessageBox.Show("Failed to load picture data. Got exception " + ee,
                    "Error",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
            }
            ...
            /*            if (_openpictureFileDialog.ShowDialog() == DialogResult.OK)
                        {
                            //OKボタンがクリックされたとき
                            //選択されたファイル名を表示する
                            Console.WriteLine(_openpictureFileDialog.FileName);
                        }
            */
        }
        ...
        private void _calib5_Click(object sender, EventArgs e)
        {
            _calib_flag = 0;
        }
        private void _calib9_Click(object sender, EventArgs e)
        {
            _calib_flag = 1;
        }
        private void _selectpicture_SelectedIndexChanged(object sender, EventArgs e)
        {
            _picture_flag = _selectpicture.SelectedIndex;
        }
    }
}
